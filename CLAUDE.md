...
| `Commands/MigrateToUuidPathCommand.php` | `php artisan media:migrate-to-uuid` — for each media record, moves files from `DefaultPathGenerator`'s ID-based path to the UUID shard path. Handles main file, conversions, and responsive images. Skips records with no UUID or whose old directory is already gone (idempotent). Supports `--dry-run` and `--force`. |
